...
function validateTool(tool: unknown, index: number): ConfigValidationError[] {
  ...
  if (typeof t.name !== "string" || t.name.trim() === "") {
    errors.push({ path: `${path}.name`, message: "Tool name is required and must be a non-empty string" });
  }

  if (typeof t.command !== "string" || t.command.trim() === "") {
    errors.push({ path: `${path}.command`, message: "Tool command is required and must be a non-empty string" });
  } else {
  ...
  if (t.aliases !== undefined) {
    if (!Array.isArray(t.aliases)) {
      errors.push({ path: `${path}.aliases`, message: "Tool aliases must be an array of strings" });
    } else if (!t.aliases.every((a) => typeof a === "string")) {
      errors.push({ path: `${path}.aliases`, message: "All aliases must be strings" });
    }
  }
...
function validateTemplate(template: unknown, index: number): ConfigValidationError[] {
  const errors: ConfigValidationError[] = [];
  ...
  if (typeof t.name !== "string" || t.name.trim() === "") {
    errors.push({ path: `${path}.name`, message: "Template name is required and must be a non-empty string" });
  }

  if (typeof t.command !== "string" || t.command.trim() === "") {
    errors.push({ path: `${path}.command`, message: "Template command is required and must be a non-empty string" });
  } else {
    const safeTemplatePattern = /^[a-zA-Z0-9._\s\-"':,!?/\\|$@]+$/;
    if (!safeTemplatePattern.test(t.command.trim())) {
      errors.push({ path: `${path}.command`, message: "Template command contains unsafe characters" });
    }
  }

  if (typeof t.description !== "string" || t.description.trim() === "") {
    errors.push({ path: `${path}.description`, message: "Template description is required and must be a non-empty string" });
  }

  if (t.aliases !== undefined) {
    if (!Array.isArray(t.aliases)) {
      errors.push({ path: `${path}.aliases`, message: "Template aliases must be an array of strings" });
    } else if (!t.aliases.every((a) => typeof a === "string")) {
      errors.push({ path: `${path}.aliases`, message: "All aliases must be strings" });
    }
  }
...
export function validateConfig(config: unknown): ConfigValidationError[] {
  ...
  if (!Array.isArray(c.templates)) {
    errors.push({ path: "templates", message: "Config must have a 'templates' array" });
  } else {
    for (let i = 0; i < c.templates.length; i++) {
      errors.push(...validateTemplate(c.templates[i], i));
    }
  }
